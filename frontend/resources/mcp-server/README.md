...
安装包内（`frontend/resources/`）：

```
resources/
├── codex-computer-use.exe   # GUI 自动化后端二进制
└── mcp-server/
    ├── index.mjs            # MCP server 入口
    ├── src/sky_client.js     # JSON-RPC 客户端
    └── package.json
```

后端启动时自动释放到 `~/.Aries/plugins/mcps/computer-use/`：

```
~/.Aries/plugins/mcps/computer-use/
├── codex-computer-use.exe   # 从 resources/ 复制
├── index.mjs
├── src/sky_client.js        # exe 路径: ../codex-computer-use.exe
└── package.json
```

`sky_client.js` 从 `../codex-computer-use.exe` 查找后端二进制；
也可用 `COMPUTER_USE_EXE` 环境变量覆盖路径。
...
- Windows 10/11
- Node.js >= 18
- `codex-computer-use.exe` 位于 `resources/codex-computer-use.exe`
  （可用 `COMPUTER_USE_EXE` 环境变量覆盖路径）

安装依赖（首次）：

```bash
cd frontend/resources/mcp-server
npm install
```
...
```json
{
  "mcpServers": {
    "computer-use": {
      "command": "node",
      "args": ["~/.Aries/plugins/mcps/computer-use/index.mjs"],
      "env": {
...
> 注意：agent 自身必须具备视觉能力，否则即使 server 返回了 image block，
