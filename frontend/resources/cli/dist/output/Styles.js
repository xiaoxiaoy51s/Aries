...
export function printBannerHeader(titleText, elapsedMs) {
    console.log();
    console.log(`  ${chalk.cyan.bold(titleText)}  ${chalk.dim(`ready in ${elapsedMs}ms`)}`);
    console.log();
}
/** 打印 Banner 行（对齐 print_banner_line） */
export function printBannerLine(labelText, value) {
    console.log(`  ${chalk.green.bold('➜')}  ${chalk.bold(labelText.padEnd(12))} ${chalk.cyan(value)}`);
}
/** 打印 Banner 尾部 */
export function printBannerFooter() {
    console.log();
}
/** 打印网络监听地址（对齐 print_network_lines） */
export function printNetworkLines(port, host, tokenSuffix = '') {
