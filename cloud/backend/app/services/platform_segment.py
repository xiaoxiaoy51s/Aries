...
参照 backend/services/platform_segment.py，适配 cloud 后端 import 路径。
"""
...
_MAX_SEGMENT_LEN = 2000


def _split_text(text: str, max_len: int = _MAX_SEGMENT_LEN) -> list[str]:
    """按长度分段，尽量在换行处截断。"""
    segments: list[str] = []
    remaining = text
...
class PlatformStreamSink:
    ...
    def __init__(self, send_fn: SendFn, *, max_len: int = _MAX_SEGMENT_LEN):
...
async def push_final_reply(send_fn: SendFn, text: str) -> None:
