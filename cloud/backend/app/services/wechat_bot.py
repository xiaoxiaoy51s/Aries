"""微信 iLink Bot 监听（全局单例）—— 适配 cloud 后端

参照 backend/services/wechat_bot.py，适配 cloud 后端 import 路径。
"""
...
def _load_wechat_config(user_email: str) -> dict:
    import json
...
class WeChatRunner:
    ...
    def _load_client(self) -> bool:
        config = _load_wechat_config(self.user_email)
    ...
    async def _async_run_loop(self):
        ...
        async def _send_segment(seg: str):
            """流式推送回调：把分段内容实时发到微信（同步 HTTP 放线程池）。"""
            if not client:
                return
            try:
        ...
        while client._running and self._running:
            try:
                # 轮询新消息（非阻塞，短间隔）
                # 用 get_running_loop 获取当前 bot 线程的 loop，
                # 避免错拿主循环导致 "attached to a different loop" 错误
                    ...
                    if _current_msg_task and not _current_msg_task.done():
                        ...
                        try:
                            await asyncio.wait_for(
                                asyncio.shield(_current_msg_task), timeout=1.0
                            )
        ...
        if _current_msg_task and not _current_msg_task.done():
            _current_msg_task.cancel()
            try:
                await asyncio.wait_for(asyncio.shield(_current_msg_task), timeout=1.0)
            except (asyncio.CancelledError, asyncio.TimeoutError, Exception):
                pass
...
def start_wechat_bot(user_email: str = "") -> bool:
    global _runner, _wechat_user_email
    _wechat_user_email = user_email
    if not user_email:
        return False
    with _lock:
        if _runner and _runner.is_running():
            return True
        _runner = WeChatRunner(user_email)
        _runner.start()
...
def restart_wechat_bot(user_email: str = ""):
    global _wechat_user_email
    email = user_email or _wechat_user_email
    stop_wechat_bot()
    time.sleep(0.3)
    if email:
        start_wechat_bot(email)
