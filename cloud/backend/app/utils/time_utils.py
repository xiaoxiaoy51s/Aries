...
参照 backend/utils/time_utils.py 适配 cloud 后端。
"""
...
def normalize_local_iso(value) -> str:
...
def parse_local_iso(value) -> datetime:
...
def local_iso_to_display(value: str) -> str:
    ...
    if not value or not str(value).strip():
        return ""
    try:
        dt = parse_local_iso(value)
        return dt.strftime("%Y-%m-%d %H:%M:%S")
    except ValueError:
        return str(value)


def local_now_plus_minutes_iso(minutes: int) -> str:
    return (local_now() + timedelta(minutes=minutes)).isoformat(timespec="microseconds")
