...
/** 开始计时（若已在计时不重置起点） */
export function startStreamDuration(sessionId, messageId) {
  if (!sessionId) return
...
/** 将 __pending__ 计时绑定到真实 messageId */
export function bindStreamDuration(sessionId, messageId) {
...
/** 结束计时并冻结时长 */
export function freezeStreamDuration(sessionId, messageId) {
  if (!sessionId) return
  const key = makeKey(sessionId, messageId)
...
export function stopStreamDuration(sessionId, messageId) {
  if (!sessionId) return
...
/** 获取当前或已冻结的毫秒数 */
export function getStreamDurationMs(sessionId, messageId) {
...
export function clearSessionStreamDurations(sessionId) {
  if (!sessionId) return
