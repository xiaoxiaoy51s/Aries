...
`sky_client.js` 会从 `../../codex-computer-use.exe`（即 `resources/codex-computer-use.exe`）
查找后端二进制；也可用 `COMPUTER_USE_EXE` 环境变量覆盖路径。
...
```json
{
  "mcpServers": {
    "computer-use": {
      "command": "node",
      "args": ["<项目根目录>\\frontend\\resources\\mcp-server\\index.mjs"],
      "env": {
