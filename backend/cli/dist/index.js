...
for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
        ...
        case '--host':
            ...
            break;
        case '--log-level':
            logLevel = parseInt(args[++i], 10);
            break;
        case '--allowed-dir':
            allowedDir = path.resolve(args[++i]);
            break;
        case '--verbose':
...
wss.on('connection', (ws, req) => {
    ...
    const workDir = parsedUrl.query.work_dir || '';
    ...
    ws.on('message', (raw) => {
        ...
        switch (msg.type) {
            ...
            case 'attach': {
                ...
                outputCallback = (data) => {
                    if (ws.readyState === WebSocket.OPEN) {
                        const outMsg = { type: 'output', data, session_id: sid };
                ...
                if (replay && !reset) {
                    ...
                    if (replayData) {
                        const outMsg = { type: 'output', data: replayData, session_id: sid };
                        ws.send(JSON.stringify(outMsg));
                    }
                }
                // 发送 ready 信号
            ...
            case 'input': {
                ...
                const data = msg.data || '';
                if (data)
                    termManager.write(sessionId, data);
                break;
            }
            case 'resize': {
                if (!attached)
